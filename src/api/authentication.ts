...
export function authentication(params: Record<string, any>) {
  return $http.get('/server/web/company/getStatus', {
    params,
  });
}
...
export function authDetails(params: Record<string, any>) {
  return $http.get('/server/web/company/info', { params });
}
// 根据社会信用代码判断是否重复
export function authRepeat(params: Record<string, any>) {
  return $http.get('/server/web/company/getByCreditCode', {
    params,
    customFields: {
...
export function authSubmit(data: Record<string, any>) {
  return $http.post('/server/web/company/saveOrUpdate', data);
}

// 认证前去判断是否有重复企业
export function apiHasCompany(data: Record<string, any>) {
  return $http.post('/server/web/company/getCompanyById', data);
}
