...
export function rolelist(data: Record<string, any>) {
  return $http.post('/server/web/role/list', data);
}

//  角色树结构
export function apiMemberlist(params: Record<string, any>) {
  return $http.get('/server/web/menu/tree', { params });
}
// 添加角色
export function apiRoleAdd(data: Record<string, any>) {
  return $http.post('/server/web/role/saveOrUpdate', data);
}
// 删除角色
export function apiRoleDelete(data: Record<string, any>) {
  return $http.post(`/server/web/role/removeById?roleId=${data.roleId}`, data, {
    customFields: {
      // @ts-ignore
...
export function apiRoleUpdate(data: Record<string, any>) {
  return $http.post('/server/web/role/changeMenu', data);
}
// 查看角色详情
export function apiRoleDetails(params: Record<string, any>) {
  return $http.get('/server/web/role/info', { params });
}
